...
* Hierarchy
   ...
   [[https://cdn.rawgit.com/tpolecat/cats-infographic/master/cats.svg][typeclasses in cats]]
   ...
   Instead of using OO style to add capability via =extends=, Meow simply declare type class =Applicaitve= need to be in context =Functor[F]= exists.
...
* Prelude
  ...
  With all new Scala 3 features, implementing type classes has never been easier and concise.
