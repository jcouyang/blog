...
{{{ruby(ADT,Algebraic Datatypes)}}} has multiple constructor returns one type:
...
This is great improvement from Scala 2, since defining a ADT is much easier than before:
...
But actually the way Scala 2 using is GADT.[fn:1]

We can also do GADT using =enum=, for instance to define a =SafeList=:[fn:2]
#+BEGIN_SRC scala -n -r
  enum Size {
    case Empty
    case NonEmpty
...
What GATD provides is a much fine control of type, i.e. line [[(nil)][(nil)]] no longer returns =List[Nothing]=,
