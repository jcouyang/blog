...
** Retro
Let us retro the evolve progress of the type of =program=
...
=ReaderT[IO, Env, Int]= consists 2 layers, =IO= and =Reader[Env, Int]=, this is the layer of *Functional Programing*
...
- effectful part is factor out of program into Env (Bare Metal)
- Dependency Injections are happen in Monad context, scalable in sense of easy to break program into smaller sub program
- effects are simply ReaderT data type, which is Pure and RT
...
#+BEGIN_SRC scala
  ...
  def state[F[_]]: Alg[F, Int]
  def log[F[_]](msg: String): Alg[F, Unit]

  def doobieQuery[F[_], A](query: ConnectionIO[A]): Alg[F, A]

  def program: Alg[F, A] = for {
    env <- state
    _ <- log(s"initState is: $state")
    valueInDatabase <- doobieQuery(sql"select 32".query[Int].unique)
  } yield initState + valueInDatabase

 val interp = new AlgInterp[F[_]]{
    val state = 1
    def log(msg: String) = IO(getLogger.log(msg))
    def query[A](c: ConnectionIO[A]) = c.transact(xa)
  }
  program.run(interp).unsafeRunSync()
#+END_SRC
