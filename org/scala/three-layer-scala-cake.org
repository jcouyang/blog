...
- Dependency Injections are happened in Monad context, scalable in sense of easy to break program into smaller sub program
- effects are simply ReaderT data type, which is Pure and RT
- lots of benefits for free from ReaderT's typeclasses instances, such as =Monoid=, =Applicative=, =MonadError=...
...
* 3 Layer Cake
We now have a solid of 3 Layer Scala Cake base made of =ReaderT=
