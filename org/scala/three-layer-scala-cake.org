...
#+BEGIN_SRC scala
  def program: ReaderT[IO, Env, Int] = for {
    ...
    valueInDatabase <- query(sql"select 32".query[Int].unique)
  } yield initState + valueInDatabase
#+END_SRC
...
Now we have 2 layers, =IO= and =Reader[Env, Int]=
...
#+BEGIN_SRC scala
  ...
  .unsafeRunSync() // <- Int
#+END_SRC
so we need to run =Reader=, and then =IO=
